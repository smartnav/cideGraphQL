var Schools = require('./../../services/schools');
...
console.log("inside", req.method);
    ...
    switch (reqMethod) {
      case 'POST':
          Schools.create(req.body, (err, response) => {
              if(!err){
      ...
      case 'PUT':
          ...
          Schools.update({_id : req.query.schoolId}, req.body, {new : true, upsert : true}, (err, response) => {
              if(!err){
                  res.send(Common.createdResponse('Successfully saved!', response));
              }else {
                  res.send(Common.errorResponse('forbidden', err));
      ...
      case 'GET':
          if(req.query.schoolId){
            Schools.find(1, {_id : req.query.schoolId}, (err, response) => {
                if(!err){
      ...
      case 'DELETE':
          if(req.query.schoolId){
            Schools.delete({_id : req.query.schoolId}, (err, response) => {
                if(!err){
